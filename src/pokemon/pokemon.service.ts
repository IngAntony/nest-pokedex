...
export class PokemonService {
  ...
  async findOne(term: string) {
    ...
    if( !pokemon ){
      throw new NotFoundException(`Pokemon con nro, name or id "${ term }" not found`)
  ...
  async remove(id: string) {
    ...
    if( deletedCount === 0)
      throw new ForbiddenException(`Pokemon with id ${ id } not exists`)
    return;
  }
  ...
  private handleExceptions( error: any, method: string){
    ...
    console.log(error)
    throw new InternalServerErrorException(`Cant't ${method} pokemon - Check de server logs`)
